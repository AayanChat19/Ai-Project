"""
Evaluation Script 3: Your System - RAG Local Only
Tests: OpenAI + Gemini + Local Evidence + Claim Analysis (no web search)
"""
...
def test_rag_local(test_cases: List[Dict]) -> List[Dict]:
    """Test your system with RAG (local only, no web search)"""
    print("\n" + "="*80)
    print("TESTING YOUR SYSTEM: RAG Local + Claim Analysis")
    print("="*80 + "\n")
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"    {result_symbol} Score: {score:.1f}/10, Evidence: {evidence_count}, Expected: {case['expected_label']}, Got: {predicted}")
...
def main():
    print("\n" + "="*80)
    print("YOUR SYSTEM EVALUATION - RAG Local")
    print("Uses OpenAI + Gemini tokens + Local RAG")
    print("="*80)
    ...
    # Run test
    results = test_rag_local(test_cases)
    ...
    output = {
        "configuration": "your_system_rag_local",
        "timestamp": timestamp,
        "metrics": metrics,
        "ece": float(ece),
        "results": results
    ...
    filename = f"results_rag_local_{timestamp}.json"
